...
from rvc.modules import utils
from rvc.modules.data_utils import TextAudioLoaderMultiNSFsid, TextAudioLoader, \
    DistributedBucketSampler, TextAudioCollateMultiNSFsid, TextAudioCollate, spec_to_mel_torch, mel_spectrogram_torch
from rvc.modules.infer_pack import commons
from rvc.modules.losses import kl_loss, feature_loss, generator_loss, discriminator_loss
from rvc.modules.slicer2 import Slicer
from rvc.modules.custom_pitch_extraction import pitch_extract as pe
from rvc.modules.rvc import load_audio
from rvc.modules.utils import savee
from rvc.engine.workspace import Workspace
...
def process_dataset(rvc_workspace):
    if rvc_workspace is None:
        print('No workspace loaded, please load a workspace first.')
        return
    dataset = rvc_workspace.data['dataset']
    sr = rvc_workspace.data['sr']
    dir_gt = os.path.join(rvc_workspace.space_path, '0_gt')
    dir_16k = os.path.join(rvc_workspace.space_path, '0_16k')
...
def pitch_extract(rvc_workspace):
    data = rvc_workspace.data
    space_path = rvc_workspace.space_path
    f0_method = data['f0']
...
def copy_model(rvc_workspace, model): 
    if model == 'f0':
        return 'Can\'t copy f0 model.'
    filename = rvc_workspace.name + '.pth'
    index_filename = f'{rvc_workspace.name}_added.index'
    index_path_filename = os.path.join(rvc_workspace.space_path, index_filename)
    model_path = os.path.join(rvc_workspace.space_path, 'models', model, filename)
    rvc_model_base_path = os.path.join('data', 'models', 'rvc', rvc_workspace.name)
...
def get_all_paths(rvc_workspace) -> tuple[str, str, str, str, int]:
    b = rvc_workspace.space_path
...
def train_model(rvc_workspace, base_ckpt_, epochs):
    epochs = int(epochs)
    data = rvc_workspace.data
    f0 = data['f0'] != 'none'
    ckpt_path = os.path.join(rvc_workspace.space_path, 'models')
    base_ckpt, is_base = (
    ...
    def get_save_paths(epoch, base_ckpt):
        ...
        return [os.path.join(base_ckpt, file_D), os.path.join(base_ckpt, file_G),
                os.path.join(base_ckpt, rvc_workspace.name + '.pth'), os.path.join(base_ckpt, 'train.json')]

    training_files = get_all_paths(rvc_workspace)
    ...
    if data['v'] == 1:
        from rvc.modules.infer_pack.models import (
...
def create_index(rvc_workspace):
    space_path = rvc_workspace.space_path
    data = rvc_workspace.data
    _name = rvc_workspace.name
